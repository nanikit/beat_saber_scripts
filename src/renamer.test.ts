import mapData from "../test/39ea1c7f8ecf7f927e2acd072378bc08c94f230a.json" with {
  type: "json",
};
...
Deno.test("Given a quest custom level", async (test) => {
  ...
  const path = `${customLevelPath}\\custom_level_39ea1c7f8ecf7f927e2acd072378bc08c94f230a - Copy`;
  ...
  await test.step("when give beatsaver data", async (test) => {
    const fileName = makeBeatsaverDirectory(mapData as BeatsaverMap, { path: path });
